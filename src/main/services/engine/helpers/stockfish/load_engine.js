var load_engine = (function () {
  ...
  async function spawn_worker(path, options) {
    var engine,
      worker = {};
    ...
    if (path.slice(-3).toLowerCase() === '.js') {
      ...
      if (
        process.version.substr(1, 2) >= 14 &&
        process.version.substr(1, 2) < 19
      ) {
  ...
  return function load_engine(path, options) {
    var worker = new_worker(path, options),
      engine = {
        ...
        stop_moves: () => {
          // Implementation of stop_moves method
        },
        get_cue_len: () => {
          // Implementation of get_cue_len method
